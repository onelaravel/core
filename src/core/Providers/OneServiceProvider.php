...
class OneServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        if ($this->app->bound('config')) {
            // $this->app->bind(
            //     \One\Contracts\UserRepositoryInterface::class,
            //     \One\Core\epositories\UserRepository::class
            // );
        }
    }
    ...
    public function boot()
    {
        ...
        if (is_dir(base_path('resources/lang'))) {
            $this->loadTranslationsFrom(base_path('resources/lang'), 'one');
        }

        // Load views nếu có
        // Cho phép package sử dụng các file view của ứng dụng nếu có.
        if (is_dir(base_path('resources/views'))) {
            $this->loadViewsFrom(base_path('resources/views'), 'one');
        }
        ...
        if ($this->app->runningInConsole()) {
            $this->commands([
                \One\Core\Console\Commands\PublishOneMigrationsCommand::class,
            ]);

            // Publish config file
            // Cho phép người dùng copy file cấu hình mặc định của package ra thư mục config của app để tùy chỉnh.
            $this->publishes([
                __DIR__ . '/../../config/one.php' => config_path('one.php'),
            ], 'one-config');
